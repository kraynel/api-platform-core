...
class QueryExpander
{
    ...
    public const ENABLE_RESULT_CACHE_ATTR = 'enable_result_cache';
    ...
    public function isEnabled(string $resourceClass)
    {
        return null !== $this->getDoctrineCacheConfig($resourceClass);
    }
    ...
    {
        if (null !== $enableResultCache = $cacheConfig[self::ENABLE_RESULT_CACHE_ATTR] ?? null) {
            if (!\is_array($enableResultCache)) {
                throw new RuntimeException(sprintf('Attribute value %s should be an array', self::ENABLE_RESULT_CACHE_ATTR));
            }
            if (empty($enableResultCache) || 2 < \count($enableResultCache)) {
                throw new RuntimeException(sprintf('Attribute %s should at least contain one item for use. Other options are lifetime and and result cache id', self::ENABLE_RESULT_CACHE_ATTR));
            }
